...
class DataSourceMetadata:
    ...
    def from_dict(cls, input_dict: dict[str, Any]):
        # Cache supported field names to avoid recomputation on every call
        if not hasattr(cls, "_supported_fields_set"):
            # Store as set for faster lookups
            cls._supported_fields_set = {f.name for f in dc.fields(cls)}
        # Only use existing fields when constructing
        args = {k: v for k, v in input_dict.items() if k in cls._supported_fields_set}
        return cls(**args)
